...
watch(
    ...
    debounce(() => {
        router.get(route("dashboard.employee"), pickBy(data.params), {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
    ...
    <Table class="shadow-md rounded-md mt-4">
        ...
        <template #table-head>
            <tr>
                ...
                <th class="p-4 cursor-pointer" @click="order('code')">
                    <div class="flex justify-between items-center">
                        <span>{{ lang().label.code }}</span>
                        <ChevronUpDownIcon class="w-4 h-4" />
                    </div>
                </th>
                <th class="p-4 cursor-pointer" @click="order('name')">
                    <div class="flex justify-between items-center">
                        <span>{{ lang().label.name }}</span>
                        <ChevronUpDownIcon class="w-4 h-4" />
                    </div>
                </th>
                <th class="p-4 cursor-pointer" @click="order('gender')">
                    <div class="flex justify-between items-center">
                        <span>{{ lang().label.Gender }}</span>
                        <ChevronUpDownIcon class="w-4 h-4" />
                    </div>
                </th>
        ...
        <template #table-body>
            <template v-if="employeeTable.data?.length">
                <tr v-for="(employee, index) in employeeTable.data" :key="employee.id"
                    class="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-200/30 hover:dark:bg-slate-900/20">
                    <td class="whitespace-nowrap px-2 py-1 text-center">{{ index + 1 }}</td>
                    <td class="whitespace-nowrap px-4 py-1">{{ employee.code || '-' }}</td>
                    <td class="whitespace-nowrap px-4 py-1">{{ employee.name || '-' }}</td>
                    <td class="whitespace-nowrap px-4 py-1">{{ props.genders[employee.gender] || '-' }}</td>
                    <td class="whitespace-nowrap px-4 py-1">{{ props.statuses[employee.status] || '-' }}</td>
                    <td class="px-4 py-1">{{ employee.created_at || '-' }}</td>
                </tr>
            </template>
            <tr v-else>
        ...
        <template #table-pagination>
            <TablePagination :filters="data.params" :links="props.employeeTable" />
        </template>
    </Table>
</template>
