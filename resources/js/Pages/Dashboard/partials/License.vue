...
watch(
    ...
    debounce(() => {
        router.get(route("dashboard.licenses"), pickBy(data.params), {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
<template>
    <Table class="shadow-md rounded-md mt-4">
        ...
        <template #table-body>
            <template v-if="licensesTable.data?.length">
                <tr v-for="(data, index) in licensesTable.data" :key="data.id"
        ...
        <template #table-pagination>
            <TablePagination :filters="data.params" :links="props.licensesTable" />
        </template>
    </Table>
</template>
