...
<template>
    <component :is="LayoutComponent" :title="lang().label.license">
        <template #title>
            <span>{{ lang().label.license }}</span>
        </template>
            ...
            <div class="w-full mx-auto sm:px-6 lg:px-8">
                <div v-if="userRole === 'superuser'">
                    ...
                    <div class="grid gap-2 md:grid-cols-12">
                        ...
                        <div class="md:col-span-3 col-span-12 grid sm:grid-cols-2 md:grid-cols-1 gap-2">
                            <div class="bg-white dark:bg-slate-800 shadow-md rounded-md p-4 flex flex-col items-center">
                                <h2 class="text-lg font-semibold text-center pb-2">{{
                                    lang().label.license_validity
                                    }}</h2>
                                <PieChart :data="pieCharts.licenseValidity"
                                          class="w-auto h-auto max-w-[85%] max-h-[85%] mx-auto aspect-square"/>
                            </div>
                            <div class="bg-white dark:bg-slate-800 shadow-md rounded-md p-4 flex flex-col items-center">
                                <h2 class="text-lg font-semibold text-center pb-2">{{
                                    lang().label.license_ownership
                                    }}</h2>
                                <PieChart :data="pieCharts.licenseOwnership"
                                          class="w-auto h-auto max-w-[85%] max-h-[85%] mx-auto aspect-square"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 bg-white dark:bg-slate-800 p-4 shadow-md rounded-md">
                        <h2 class="text-xl font-semibold mb-4">{{ lang().label.license }}</h2>
                        <License
                                :licensesTable="licenseTable"
                                :types="types"
                                :statuses="statuses"
                                :filters="filters"
                                :perPage="perPage"/>
                ...
                <div v-else-if="userRole === 'participant'">
                    <div class="container mx-auto max-w-fit mt-6">
                        ...
                        <p class="text-gray-600">Ini adalah dashboard pengguna biasa.</p>
                    </div>
                </div>
            </div>
        </div>
