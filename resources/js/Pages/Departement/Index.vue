...
import Create from "@/Pages/Departement/Create.vue";
import Edit from "@/Pages/Departement/Edit.vue";
import Delete from "@/Pages/Departement/Delete.vue";
import DeleteBulk from "@/Pages/Departement/DeleteBulk.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("department.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.departements?.data.forEach((departement) => {
            data.selectedId.push(departement.id);
        });
    }
};
const select = () => {
    if (props.departements?.data.length == data.selectedId.length) {
                    ...
                    <Table>
                        <template #table-action>
                            <div class="flex shrink-0 rounded overflow-hidden">
                                ...
                                <Create v-show="can(['departement create'])" :title="props.title" :statuses="props.statuses"/>
                                <DeleteBulk v-show=" data.selectedId.length != 0 && can(['departement delete'])
                                    "
                        ...
                        <template #table-body>
                            <tr v-for="(departement, index) in departements.data" :key="index" class="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-200/30 hover:dark:bg-slate-900/20">
                                <td class="whitespace-nowrap px-2 py-1">
                                    <input class="rounded dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-primary dark:text-primary shadow-sm focus:ring-primary/80 dark:focus:ring-primary dark:focus:ring-offset-slate-800 dark:checked:bg-primary dark:checked:border-primary"
                                        type="checkbox" @change="select" :value="departement.id"
                                        v-model="data.selectedId"/>
                                </td>
                                <td class="whitespace-nowrap px-2 py-1 text-center">{{ ++index }}</td>
                                <td class="whitespace-nowrap px-2 py-1">{{ departement.name }}</td>
                                <td class="whitespace-nowrap px-4 py-1">{{ props.statuses[departement.status] || '-' }}</td>
                                <td class="whitespace-nowrap px-2 py-1">{{ departement.created_at }}</td>
                                <td class="whitespace-nowrap flex justify-end px-2 py-1">
                                    <div class="flex w-fit rounded overflow-hidden">
                                        <Edit v-show="can(['departement update'])" :title="props.title" :statuses="props.statuses" :departement="data.departement" @open="data.departement = departement"/>
                                        <Delete v-show="can(['departement delete'])" :title="props.title" :departement="data.departement"@open="data.departement = departement"/>
                                    </div>
                                </td>
                            </tr>
                        </template>
                        <template #table-pagination>
                            <TablePagination :links="props.departements" :filters="data.params"/>
                        </template>
                    </Table>
                </div>
            </div>
